...
impl ChordNode {
    ...
    pub(crate) fn find_next_routing_hop(&self, hdr: &ChordMessageHeader) -> ChordRoutingOutcome {
        ...
        let mut last_node_id = self.id;
        let mut last_node_addr = self.addr;
        for succ in self.rs.succ_table.iter() {
            if succ.is_none() {
                return if last_node_id == self.id {
                    ChordRoutingOutcome::Accept
                } else {
                    ChordRoutingOutcome::Forward(last_node_addr)
                }
            }

            let succ = succ.unwrap();
            if chord_id_in_range(self.rs.finger_table.len(), &dst_id, last_node_id..succ.id) {
                return ChordRoutingOutcome::Forward(succ.addr)
            }
            last_node_id = succ.id;
            last_node_addr = succ.addr
        }

        last_node_id = self.id;
        last_node_addr = self.addr;
        for pred in self.rs.pred_table.iter() {
            ...
            if chord_id_in_range(self.rs.finger_table.len(), &dst_id, (pred.id + 1)..=last_node_id) {
                return ChordRoutingOutcome::Forward(last_node_addr)
            }
            last_node_id = pred.id;
            last_node_addr = pred.addr;
        }

        let mut addr = self.addr;
        for i in 0..self.rs.finger_table.len() {
            if chord_id_in_range(self.rs.finger_table.len(), &dst_id, 2_u128.pow(i as u32)..2_u128.pow((i + 1) as u32)) {
                addr = self.rs.finger_table.get(i).unwrap().unwrap().addr;
            }
        }
        ChordRoutingOutcome::Forward(addr)
    }
