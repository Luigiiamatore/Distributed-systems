#[cfg(test)]
pub(crate) mod tests {
    use tokio::sync::mpsc::{unbounded_channel as unbounded, UnboundedReceiver as Receiver, UnboundedSender as Sender};
    ...
    pub(crate) async fn build_system<const N: usize>(
        ...
        let senders: [[Sender<Operation>; N]; N] = [[(); N]; N].map(|arr| arr.map(|()| senders_iter.next().unwrap()));
        let mut receivers_iter = channels_out.into_iter();
        let receivers: [[Receiver<Operation>; N]; N] = [[(); N]; N].map(|arr| arr.map(|()| receivers_iter.next().unwrap()));
